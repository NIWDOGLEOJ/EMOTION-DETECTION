...
try:
    ...
    logger.info("Install with: pip install opencv-python deepface transformers torch accelerate")
    exit(1)
...
class QuantizedChatBot:
    ...
    def _load_model(self):
        ...
        try:
            ...
            if self.tokenizer.pad_token is None and "t5" not in self.model_name:
...
def main():
    ...
    print()
    models = {
        "1": ("microsoft/DialoGPT-small", "117M params", "Fast, conversation-focused"),
        "2": ("google/flan-t5-small", "77M params", "Good instruction following")
    }

    print("Available Models (choose by number):")
    for key, (name, params, desc) in models.items():
        print(f"{key}. {name} ({params}) - {desc}")
    ...
    try:
        ...
        print("2. Install dependencies: pip install opencv-python deepface transformers torch accelerate")
